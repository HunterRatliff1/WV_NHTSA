...
Returning to the discussion above about limiting the search space, if we tried to find the distance for *all cases* to *all hospitals*, it would be a `r nrow(df_cases)` by `r nrow(hospitals)` matrix for the travel times, which would be `r scales::comma(nrow(df_cases) * nrow(hospitals))` results!
...
The compromise I landed on was to "chunk" things into smaller groups, namely by county. So instead of determining the distance from *all cases* to *all hospitals*, I loop through the following [for each county]{.underline}:

1.  Look at all of the cases in a county

2.  Find the `r formals(fxn1)[["n_closest"]]` closest hospitals for each case within the county. The list of all of these hospitals becomes the hospitals we will look at for that county

3.  For each county, query the driving time from all of the cases (in that county) to the hospitals identified in step 2
...
fxn3 <- function(df_cases, df_hosp, output="travel"){
  
  # Make sure provided output is valid
  output <- rlang::arg_match(output, c("travel", "hosp"))
  
  hosp <- fxn1(geo_polys = df_cases, 
               geo_points = df_hosp, 
               name_polys = "case_id") %>%
    semi_join(df_hosp, ., by = join_by(ID))
  
  if(output=="hosp") return(hosp)
  
  fxn2(case_geo = df_cases, hosp_geo = hosp)
}

# Create a version of fxn3 
...
```{r travel_times, eval=FALSE, message=TRUE}
...
temp.geo <- wv_counties

fxn1(temp.geo, hospitals)  %>%
  ...
  # of that geography
  inner_join(temp.geo, by = join_by(GEOID)) %>%
  mutate(geometry = st_centroid(geometry),
         geometry = st_buffer(geometry, dist=Distance)) %>%
  st_as_sf() %>%
  ggplot() + 
  geom_sf(color="red") +
  geom_sf(data=temp.geo, alpha=0.5) +
  geom_sf(data=hospitals, alpha=0.5, color="blue") 
